...
SELECT Models.name, brand_name, headquarters 
   FROM Models JOIN Brands ON (Brands.name = brand_name)
...
SELECT * FROM Models LEFT JOIN Brands
...
SELECT Models.brand_name, Models.name, founded FROM Models
...
-- A left join takes all of the selected items on the left side (the side mentioned after "FROM")
-- and lists them with corresponding information on the right side, whether information for 
-- those items exists or not. An inner join matches corresponding rows on each side, only if
-- information for those rows exists on both sides of the "ON" statement.
...
--   IIHS Safety Award    2015      # get the ``id`` of the 2015 Subaru Outback
    ...
    (SELECT id FROM Models AS m WHERE m.year = 2015 AND m.name = 'Malibu'));
    
    INSERT INTO Awards (name, year, winner)
        VALUES ('IIHS Safety Award', 2015, 
    (SELECT id FROM Models AS m WHERE m.year = 2015 AND m.name = 'Outback'));
